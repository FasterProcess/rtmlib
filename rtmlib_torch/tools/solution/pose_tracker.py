...
def pose_to_bbox_torch(keypoints: torch.Tensor, expansion: float = 1.25) -> torch.Tensor:
    """Get bounding box from keypoints.

    Args:
        keypoints (torch.Tensor): Keypoints of person.
        expansion (float): Expansion ratio of bounding box.

    Returns:
        torch.Tensor: Bounding box of person.
    """
    x = keypoints[:, 0]
    y = keypoints[:, 1]
    bbox = torch.tensor([x.min(), y.min(), x.max(), y.max()])
    center = torch.tensor([bbox[0] + bbox[2], bbox[1] + bbox[3]]) / 2
    bbox = torch.cat([
        center - (center - bbox[:2]) * expansion,
        center + (bbox[2:] - center) * expansion
    ])
...
class PoseTracker:
    ...
    def __call__(self, image: torch.Tensor):
        ...
        b, h, w, c = image.shape
        assert c == 3, 'Input image must have 3 channels (BGR).'

        frame_indices = torch.arange(self.frame_cnt, self.frame_cnt + b)
        valid_mask = (frame_indices % self.det_frequency == 0)

        det_image = image[valid_mask]

        if det_image.shape[0] > 0:
            bboxes, box_scores = self.det_model(det_image) # list

        box_idx = 0
        keypoints_all = []
        scores_all = []
        for i, det_flag in enumerate(valid_mask):
            if det_flag:
                bbox = bboxes[box_idx]
                box_idx += 1
            else:
                bbox = self.bboxes_last_frame

            # print(bbox)

            keypoints, scores = self.pose_model(image[i:i+1], bboxes=bbox)
